...
try {
    ...
    $stmtUpdate = $conn->prepare("UPDATE ganado SET prenez_numero = ?, prenez_fecha = ? WHERE id = ?");
    $stmtUpdate->bind_param("ssi", $prenez_numero, $prenez_fecha, $id);
    ...
    $stmtInsert = $conn->prepare("INSERT INTO h_prenez (tagid, prenez_numero, prenez_fecha) VALUES (?, ?, ?)");
    $stmtInsert->bind_param("iss", $tagid, $prenez_numero, $prenez_fecha);
