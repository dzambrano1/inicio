...
if (
    !isset($data['id']) ||
    !isset($data['bano']) ||
    !isset($data['bano_fecha']) ||
    !isset($data['bano_costo'])
) {
    echo json_encode([
        'success' => false,
...
$bano_fecha = $conn->real_escape_string($data['bano_fecha']);
$bano_costo = floatval($data['bano_costo']);
...
try {
    ...
    $stmtUpdate->bind_param("ssdi", $bano, $bano_fecha, $bano_costo, $id);
    if (!$stmtUpdate->execute()) {
        throw new Exception("Error al ejecutar la actualización: " . $stmtUpdate->error);
    }
    $stmtUpdate->close();

    // Insert into h_bano table
    $stmtInsert = $conn->prepare("INSERT INTO h_bano (tagid, bano, bano_fecha, bano_costo) VALUES (?, ?, ?, ?)");
    $stmtInsert->bind_param("sssd", $tagid, $bano, $bano_fecha, $bano_costo);
    if (!$stmtInsert->execute()) {
        throw new Exception("Error al insertar en h_bano: " . $stmtInsert->error);
    }
    $stmtInsert->close();
