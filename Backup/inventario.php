...
<div style="max-width: 1400px; margin: 0px auto; padding: 0 10px;">
    ...
    <!-- Grafico Promedio mensual de Peso Carnico -->
    <div style="max-width: 1300px; margin: 40px auto;">
        <h3 style="text-align: center;">Pesos Promedios Carne</h3>
        <canvas id="monthlyAverageWeightChart"></canvas>
    </div>
    ...
    function openHistory(tagid) {
        // Redirect to historial.php with tagid as a query parameter
...
// SQL query to get the average weight per month
$sql = "
    SELECT 
        DATE_FORMAT(peso_fecha, '%Y-%m') AS month,
        AVG(peso) AS average_weight
    FROM 
        ganado
    GROUP BY 
        month
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $labels[] = $row['month']; // Store the month in 'YYYY-MM' format
        $data[] = (float)$row['average_weight']; // Store the average weight
    }
}
?>

<?php
// After building the WHERE clause based on selected filters

// SQL query to get the average weight per month with filters
$sqlAverageWeight = "
    SELECT 
        DATE_FORMAT(peso_fecha, '%Y-%m') AS month,
        AVG(peso) AS average_weight
...
if (!empty($whereClause)) {
    $sqlAverageWeight .= " WHERE " . implode(' AND ', $whereClause);
}
...
$resultAverageWeight = $conn->query($sqlAverageWeight);

$labels = [];
$data = [];

if ($resultAverageWeight->num_rows > 0) {
    while ($row = $resultAverageWeight->fetch_assoc()) {
        $labels[] = $row['month']; // Store the month in 'YYYY-MM' format
        $data[] = (float)$row['average_weight']; // Store the average weight
    }
}
...
<script>
const ctxMonthlyAverage = document.getElementById('monthlyAverageWeightChart').getContext('2d');
const monthlyAverageWeightChart = new Chart(ctxMonthlyAverage, {
    type: 'line',
    data: {
        labels: <?php echo json_encode($labels); ?>, // Use PHP to output the labels
        datasets: [{
            label: 'Pesos Promedio (Kg)',
            data: <?php echo json_encode($data); ?>, // Use PHP to output the data
